...
- Version: `2.3.0`
- Helm chart package: `helm/playsbc-2.3.0.tgz`
- Project license: MIT
- Chart version: `2.3.0`
...
shasum -a 256 release/helm/playsbc-2.3.0.tgz > release/helm/playsbc-2.3.0.tgz.sha256
```
...
PYTHONPYCACHEPREFIX=/private/tmp/playsbc-pycache python3 tools/run_k8s_regression_job.py \
  --all-profiles \
  --runner-image ghcr.io/sudheerkumarvatrapu/playsbc-k8s-regression:2.3.0 \
  --sipp-image ghcr.io/sudheerkumarvatrapu/playsbc-sipp:2.3.0 \
  --playsbc-image ghcr.io/sudheerkumarvatrapu/playsbc:2.3.0 \
  --set-playsbc-image \
...
They also wait for Azure LoadBalancer ingress before the regression Job starts. For AKS profile runs in v2.3.0 and later, the wrapper waits for both SIP and RTP public LoadBalancer ingress, validates UDP media ports `30000-30049`, and records RTPengine advertised-IP alignment evidence when RTPengine pods are present.
