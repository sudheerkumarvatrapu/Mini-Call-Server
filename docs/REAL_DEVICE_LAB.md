...
- Dialing `1002` from an ATA returns address-incomplete/failed routing: check whether the INVITE Request-URI is only the SBC public IP. PlaySBC v1.6.3 falls back to the `To` header user for that proxy-style INVITE.
...
- Some hardphones send `INVITE sip:<proxy-ip>:5062` and keep the dialed extension in `To: <sip:1002@...>`. PlaySBC v1.6.3 routes that using `1002` instead of treating the public IP as the called user.
