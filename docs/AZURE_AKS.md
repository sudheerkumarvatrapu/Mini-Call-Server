...
- OBi1022 can send a proxy-style INVITE where the Request-URI is the AKS public IP and the dialed extension is only in the `To` header. PlaySBC v1.6.2 routes that call using the `To` user instead of treating the public IP as the called user.
