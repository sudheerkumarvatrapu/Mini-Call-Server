...
This is also the current development safety gate. If you upgrade the local chart/app version before publishing images, run this local-image command; it uses the source tree and does not require the matching `ghcr.io/...:<version>` images to exist yet.
